    ...
    <v-col cols="9" md="10" xs="9">
      <v-row no-gutters class="fill-height">
        <v-col cols="12" md="6" xs="12" class="d-flex align-center justify-center"
          ><span class="text-truncate px-2">{{ playerStore.getCurrentTrack.name + ' - ' + playerStore.getArtistName }}</span></v-col
        >
        <v-col cols="6" md="4" xs="6" class="d-flex align-center justify-center">
          ...
          <v-icon v-if="!playerStore.playerState.paused" @click="spotifyPlayer.togglePlay()" icon="mdi-pause" class="mx-2" />
        ...
        <v-col cols="6" md="2" xs="6" class="d-flex align-center justify-center"
          ><v-slider min="0" max="1" v-model="volume" hide-details color="secondary"></v-slider></v-col
      ></v-row>
    </v-col>
  </v-row>
</template>
